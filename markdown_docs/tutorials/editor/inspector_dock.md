...
The values with different values than their original values will have a revert icon (![undo](img/inspector_dock_revert.webp)).
...
- **Expand Non-Default**: Only expands the sections where the original value is different
  than the current value (the properties with a revert icon (![undo](img/inspector_dock_revert.webp))).
