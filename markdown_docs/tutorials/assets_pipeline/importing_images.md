...
- BMP (`.bmp`) - All pixel formats are supported, but 
...
![Import options in the Import dock after selecting an image in the FileSystem dock](img/importing_images_import_dock.webp)
...
- **DPITexture:** Only available for SVG images. Similar to Texture2D, but can be
  ...
  to be reimported.
  See [Font and image oversampling](../rendering/multiple_resolutions.md#doc-multiple-resolutions-font-and-image-oversampling) for details.
...
advantages and disadvantages (![good](img/good.png) = best, ![bad](img/bad.png) = worst):
...
![Previewing a texture in the Inspector](img/importing_images_inspector_preview.webp)
...
![Normal map with standard VRAM compression (left) and with RGTC VRAM compression (right)](img/importing_images_normal_map_rgtc.webp)
